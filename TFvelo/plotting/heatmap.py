...
def heatmap(
    ...
    **kwargs,
    ...
    df = pd.DataFrame(df.values[:, int(filter_start * n_genes): int(filter_end * n_genes)], columns=df.columns[int(filter_start * n_genes): int(filter_end * n_genes)])
    strings_to_categoricals(adata)
    '''
    with open("figures/selected_genes.txt","w") as f:
