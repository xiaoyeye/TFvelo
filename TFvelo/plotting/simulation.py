...
def show_full_dynamics(
    ...
    if key != "true":
        _, WX_t, y_t, t_points = compute_dynamics(
            adata, basis, key
        )
        if not show_full_dynamic:
            data = adata[:,basis].layers['fit_t_raw'].reshape(-1)
            if not 0 in data:
            ...
            if (blank_start_id==0) or (blank_start_id==len(intervals)-1):
                nonblank_idx = (t_points>sorted_data[1]) & (t_points<sorted_data[len(intervals)-1])
                mid_idx = int((nonblank_idx).sum()/2)
            else:
                nonblank_idx = (t_points<sorted_data[blank_start_id]) | (t_points>sorted_data[blank_start_id+1])
                mid_idx = int((nonblank_idx).sum()/2) - (t_points>sorted_data[blank_start_id+1]).sum()
