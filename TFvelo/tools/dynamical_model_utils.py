...
def compute_divergence(
    ...
    **kwargs,
    ...
    # adjust increments of omega, omega_ to avoid meaningless jumps
    if constraint_time_increments:
        ...
        WX = use_model(WX_model, x, method=WX_method, bias=delta) 
        dist_WX, dist_y = (WX - WX_t), (y - y_t) 

        res = np.array([dist_WX ** 2 + dist_y ** 2])
    ...
    if mode in {"assign_timepoints", "time", "t"}:
        ...
        varx = (
            np.nanmean(dist_data, 0) - np.nanmean(np.sign(dist_y) * np.sqrt(dist_data), 0) ** 2
        )
        ...
        if clusters is not None:
            ...
            for cat in clusters.cat.categories:
                ...
                distx_cat = dist_WX[idx_cat] ** 2 + dist_y[idx_cat] ** 2
        ...
        if time_connectivities:
            ...
            o_ = (res < 2) * (t >= t_)
            omega, alpha, u0, s0 = vectorize(t, t_, alpha, beta, gamma, weight)
            ut, st = SplicingDynamics(alpha=alpha, beta=beta, omega=omega, theta=theta, gamma=gamma).get_solution(
                omega, stacked=False
            )
            ut_, st_ = ut, st
...
def curve_dists(
    ...
    if u0_ is None or s0_ is None:
        u0_, s0_ = SplicingDynamics(alpha=alpha, beta=beta, omega=omega, theta=theta, gamma=gamma).get_solution(
            t_, stacked=False
        )

    x_obs = np.vstack([u, s]).T
    std_x = np.vstack([std_u / scaling, std_s]).T
    t0 = omega_inv(np.min(u[s > 0]), u0=u0_, alpha=0, beta=beta, weight=weight)
    ...
    curve_t = SplicingDynamics(alpha=alpha, beta=beta, omega=omega, theta=theta, gamma=gamma).get_solution(
        tpoints
    )
...
class BaseDynamics:
    ...
    def update_WX(self, alpha, beta, omega, theta, gamma, method='NN'): 
        ...
        if method == 'constant':
            ...
            if 1: # [X,1]
                ...
                #lb = np.append(np.zeros(self.n_TFs), -20)
                #hb = np.append(20*np.ones(self.n_TFs), 20)
                #res = lsq_linear(input_x, WX_t, bounds=(lb, hb), lsmr_tol='auto', verbose=0)
                res = lsq_linear(input_x, WX_t, bounds=(-20, 20), lsmr_tol='auto', verbose=0)
                model = res.x.reshape(-1, 1)
                model = [model[:-1], model[-1][0]]
            else: # [X]
                res = lsq_linear(self.x_f, WX_t, bounds=(0, 10), lsmr_tol='auto', verbose=0) #(-10, 10)
                model = res.x.reshape(-1, 1)
                model = [model, 0]
            #print(model)
            #print(res.cost)
            ...
            #input_data=preprocessing.scale(self.x_f)
            kernel = Matern() + WhiteKernel()#构造核函数形式
            model= GaussianProcessRegressor(kernel=kernel,#指定GP协方差函数的内核
                                            ...
                                            copy_X_train=True,
                                            random_state=None)
            model = model.fit(self.x_f, WX_t.reshape(-1,1))
            WX_f = model.predict(self.x_f).reshape(-1)
            ...
            def run_NN(X_train, y_train, n_epoch=100, lr=0.02):
                X_train = torch.Tensor(X_train)
                y_train = torch.Tensor(y_train)
                model = Net(in_channels=X_train.shape[1])
                ...
                for it in range(n_epoch):
                    ...
                    loss = loss_fn(y_pred, y_train) + 0.01*reg
                    #print(it, loss.item())
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                output_train = model(X_train)
                output_train = output_train.detach().numpy().reshape(-1)
                return model

            model = run_NN(self.x_f, WX_t.reshape(-1,1))
    ...
    def get_curve_likelihood(self):
        ...
        dist, dist_ = curve_dists(u, s, alpha, beta, gamma, weight, t_, **kwargs)
    ...
    def get_ut(self, **kwargs):
        t, t_, alpha, beta, gamma, weight, scaling = self.get_vals(**kwargs)
        omega, alpha, u0, s0 = vectorize(t, t_, alpha, beta, gamma, weight)
        return unspliced(omega, u0, alpha, beta)

    def get_st(self, **kwargs):
        t, t_, alpha, beta, gamma, weight, scaling = self.get_vals(**kwargs)
        omega, alpha, u0, s0 = vectorize(t, t_, alpha, beta, gamma, weight)
        return spliced(omega, s0, u0, alpha, beta, gamma, weight)

    def get_vt(self, mode="soft_eval"):
        alpha, beta, gamma, weight, scaling, _ = self.get_vars()
        o_, o, ut, st = self.get_divergence(mode=mode)
        return ut * weight - st * gamma
    ...
    def plot_phase(
        ...
        **kwargs,
        ...
        if np.all([x is None for x in [alpha, beta, gamma, weight, scaling, t_]]):
            refit_time = False
        t, t_, alpha, beta, gamma, weight, scaling = self.get_vals(
            t, t_, alpha, beta, gamma, weight, scaling, refit_time=refit_time
        )
        ut = self.get_ut(t=t, t_=t_, alpha=alpha, beta=beta, gamma=gamma, weight=weight) * scaling
        st = self.get_st(t=t, t_=t_, alpha=alpha, beta=beta, gamma=gamma, weight=weight)
