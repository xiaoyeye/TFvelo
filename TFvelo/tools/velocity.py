...
class Velocity:
    def __init__(
        ...
        self._adata = adata
        self.n_TFs = len(adata.varm['TFs'][0])
        if M_total is None:
            self._M_total = adata.layers["total"] if use_raw else adata.layers["M_total"]
        else:
            self._M_total = M_total
        self._M_total = make_dense(self._M_total)

        n_obs, n_vars = self._M_total.shape
        ...
        self._alpha = np.ones(n_vars, dtype=np.float32)
        self._beta = np.ones(n_vars, dtype=np.float32)
        self._tau = np.ones(n_vars, dtype=np.float32)
        self._rho = np.ones(n_vars, dtype=np.float32)
        self._gamma = np.zeros(n_vars, dtype=np.float32)
        self._weight = np.ones([n_vars, self.n_TFs], dtype=np.float32)
    ...
    def compute_stochastic(
        ...
        if mode == "bayes":
            self._offset[idx], self._offset2[idx], self._gamma[idx] = maximum_likelihood(_Ms, _Mu, _Mus, _Mss, fit_offset, fit_offset2)
        else:
            leastsq_generalized(
                _Ms,
                _Mu,
...
def velocity(
    ...
    **kwargs,
    ...
    velocity_genes, velocity_beta, velocity_gamma, velocity_weight, velocity_r2: `.var`
        parameters
    """  # noqa E501

    adata = data.copy() if copy else data
    if not use_raw and "M_total" not in adata.layers.keys():
        moments(adata)
    ...
    if mode in {"dynamical"}:
        ...
        alpha, beta, gamma, weight, scaling, t_ = get_vars(vdata)
        alpha, beta, tau, rho, gamma, weight = get_vars(alpha, beta, tau, rho, gamma, weight)
    ...
    '''
        ...
        for cat in categories:
            ...
            write_residuals(adata, vkey, velo._residual, cell_subset)  ## write velocity
