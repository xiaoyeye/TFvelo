...
def moments(
    ...
    -------
    M_total: `.layers`
        dense matrix with first order moments of total counts.
    Ms: `.layers`
    ...
    layers = [layer for layer in {"total"} if layer in adata.layers]
    ...
    if "total" not in adata.layers.keys(): # or "unspliced" not in adata.layers.keys():
        logg.warn("Skipping moments, because total counts were not found.")
    else:
